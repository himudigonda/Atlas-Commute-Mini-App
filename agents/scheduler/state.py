...
class TrafficMetrics(BaseModel):
    ...
    traffic_delay_seconds: int = Field(
        0, ge=0, description="Additional time due to traffic"
    )
...
class UserContext(BaseModel):
    ...
    destination: Optional[str] = "Airport"
    flight_number: str = Field(..., pattern=r"^[A-Z0-9]{2,}\d+$")
    target_arrival_time: Optional[datetime] = Field(
        None, description="When user wants to be at the destination"
    )
...
class CommutePlan(BaseModel):
    ...
    reasoning_trace: str = Field(
        ..., description="Short explanation of the decision logic"
    )
    notification_message: Optional[str] = Field(
        None, description="Drafted message for the user"
    )
